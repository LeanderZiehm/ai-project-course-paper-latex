...
In our evaluation, we first encountered the issue that some free LLM providers had pretty limiting rate limits. Due to time constraints, we had to skip testing Gemini. We mostly used Grok, OpenAI, and our own self-hosted Ollama, which included a few local models. However, we could not test all models on everything, again due to the time limitations.
